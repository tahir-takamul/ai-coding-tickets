...
**How we handle it.** Mobile2 inherits the same constraint for now.
...
**How we handle it.** Mobile2 keeps the same split. The orchestrator
...
**How we handle it.** Mobile2 keeps the same shape. The MDM-distribute
...
legacy `MobileApp` vs. mobile2's `00-Mobile/Modules`), the **only**
...
**How we handle it.** Mobile2 orchestrator continues to pass
`result.fullProjectName` and `result.number` opaquely. The
upload module re-uses the same translation. The current mobile2
